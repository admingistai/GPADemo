...
export default async function handler(req, res) {
  try {
    ...
    if (!contentType.includes('text/html')) {
      return res.status(400).json({ error: 'Only HTML content is supported' });
    }
    ...
    // Inject before closing body tag or at the end of HTML if no body tag
    ...
    return res.status(500).json({ 
      error: 'Failed to proxy the website',
      details: error.message
    });
  }
}
